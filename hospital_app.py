...
def run_detailed_simulation(specialty_config, simulation_days):
    ...
    for _, config in specialty_config.iterrows():
        ...
        for day in range(simulation_days):
            ...
            if current_backlog > 0 and initial_backlog > 0:
...
user_config = {}
specialty_count = len(SPECIALTY_CONFIG)

# Progress indicator
st.sidebar.info(f"📊 Configuring {specialty_count} hospital specialties")

# Collapsible specialty configuration
for specialty, defaults in SPECIALTY_CONFIG.items():
...
if hasattr(st.session_state, 'simulation_run') and st.session_state.simulation_run:
    ...
    with col4:
        stable_count = len(executive_display[executive_display['System Status'].str.contains('🟠')])
        st.metric("🟠 Stable/Monitoring", stable_count)
    ...
    **📈 Performance Monitoring**
    ...
    1. **Configure Parameters**: Adjust staffing levels and patient volumes in the sidebar
    2. **Set Time Horizon**: Choose simulation duration (30-365 days)
    3. **Run Analysis**: Execute comprehensive capacity modeling
    4. **Review Results**: Analyze performance metrics and strategic recommendations
    5. **Export Data**: Download results for presentations and further analysis
